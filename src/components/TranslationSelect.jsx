...
const TranslationSelect = ({ darkMode }) => {
            ...
            <div className="relative">
                <select
                    className={`w-full p-4 pr-10 rounded-2xl border appearance-none outline-none transition-all duration-300 cursor-pointer font-medium
                        ${darkMode
                            ? 'bg-slate-800 border-slate-700 text-white focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 hover:bg-slate-750'
                            : 'bg-white border-slate-200 text-slate-900 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 hover:border-slate-300 shadow-sm hover:shadow-md'
                        }`}
                    value={selectedTranslation || ''}
                    onChange={(e) => setSelectedTranslation(Number(e.target.value))}
                >
                    {translations.map((translation) => (
                        <option key={translation.id} value={translation.id} className={darkMode ? 'bg-slate-800' : 'bg-white'}>
                            {translation.name}
                        </option>
                    ))}
                </select>
